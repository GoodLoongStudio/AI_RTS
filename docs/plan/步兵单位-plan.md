...
> 2026-09-06 更新：动画已改为原厂 50 骨架 v3，运行时使用 `assets/models/polygon-scifi/Infantry_native_v3.glb`。旧重绑 FBX、v2 动画及其生成/审核产物已清理；当前重建入口和验证边界见 [4006 原厂骨架动画基线](4006原厂骨架动画基线-plan.md)。以下分期步骤保留开发历史，不再作为旧素材或程序化动画的使用指引。
...
- ✅ 当前动画基线：`初选素材包/绑骨管线/4006/原厂骨架_v3/`，一个标准士兵、七段动画；其他角色尚未按新方案批量生成。
- ✅ 原始角色/武器 FBX 和动作源保留，新管线位于 `初选素材包/绑骨管线/native_pipeline/`。
