...
3. 每人点准备；`connected_human_count ∈ [2, 4]` 且槽内全员准备 → 开局。**「立即开局」按钮**：≥1 人即可跳过等待强制开局，空槽默认保持空；房主可在大厅显式添加 AI。未联网时点击会自动连接云服，单人也能开房玩。
...
| 阶段 | 可玩标准 | 状态 |
|---|---|---|
| 架构冻结（本文） | 复核 AI 无否决项，或否决项被设计师拍板 | ✅ 2026-08-29 复核完成，P0/P1 当日实施 |
| 本机双进程 | 两窗口互相看见移动 | ✅ 2026-08-29 |
| 云上 2 人 | 防火墙放行 UDP 24567；两台不同网络电脑打完 Plain & Simple | 专用服已上云运行（git bundle 增量部署 + systemd 托管）；⏳ 防火墙放行与公网试玩 |
| 3–4 人 | 空槽 AI 或空着；掉线算负、对局继续（不重连） | 大厅可显式切换空槽/AI；单人立即开局默认不补 AI |
...
## Related Plans（2026-08-30 增）

- `docs/plan/规则AI智能化改造-plan.md`：空槽规则 AI（SimpleClairvoyantAI）的行为升级——扩张经济、主动进攻、波次与撤退、防御响应、难度分级。2026-08-30 立项并完成一轮外部评审（评审确认：attack-move 指令模拟层已存在，仅 `RuleAiCommandGateway` 未暴露）。规则 AI 只在权威服务器进程思考（`is_client_puppet()` 早退），该升级不触碰本文定义的联机协议与快照契约；唯一联机影响是 AI 单位总量上升放大 10Hz 全量快照体积（见该 plan 的 Risk 1）。
- `docs/plan/服务器Agent与局服-yyp_test-plan.md`：LLM「AI 副官」网关——与规则 AI 是两套系统，不要混淆。
