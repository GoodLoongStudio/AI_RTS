...
| 包 2–3 | 本机双进程 | 两个窗口能互相看见对方单位移动 | ✅ 双进程冒烟验收通过（2026-08-30：headless 专用服+冒烟客户端，join→AI 补位开局→go-live→16 单位快照全部到位，SMOKE_OK；冒烟钩子 `--smokeclient` 见 Online.gd/SmokeClient.gd）。人工双窗口互看可作补充，不阻塞 |
| 包 4 | 云上 2 人 | 两台不同网络电脑连公网 IP 打完一局 Plain & Simple | ✅ 防火墙已放行（用户 2026-08-30）；云服已部署确定性命名+冒烟钩子（5187bd1）并重建 C#；公网自动化冒烟通过（SMOKE_OK units=16，本机客户端→101.43.121.102:24567 全链路：join→AI 补位开局→go-live→快照插值）。真人 2 机试玩待约 |
| 收尾 | 3–4 人 | 四人槽位、空槽 AI 或禁用；掉线算负、对局继续（不重连） | AI 补位已实施（start_solo，2026-08-30）；大厅「填 AI」开关 ✅（槽位行 ＋AI/撤AI 按钮已有，`host_set_slot_kind`，2026-08-30 核对） |
...
- ✅ 本机双进程复验（2026-08-30 冒烟通过：SMOKE_OK units=16；过程中修复两个 P0 级缺陷——①Match.gd 玩家/单位自动命名跨进程漂移导致清单必分叉，改为 Player_%d/Unit_%d 确定性命名；②AI 控制器解析错误致 Match 无法加载）
- ⏳ 2 人公网试玩、3–4 人试玩（前置：防火墙放行 UDP 24567，人工）
