...
1. 新增 `ground_tile_meters`（默认 50 世界米），用 `world_xz = p.xz * showcase_world_scale` 采样，不跟 review 坐标绑死。
2. 性能模式：`image25_macro_sand` + `dirt_aerial` 按 50 世界米平铺。不用 `sand_uniform`（几乎无地景结构）。
