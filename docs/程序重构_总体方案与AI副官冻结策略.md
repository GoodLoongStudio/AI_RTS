...
- 战役通过显式开关保留现有演示；
- 提供最小化的 AI 副官界面显示/隐藏按钮与设置项，隐藏后不得继续遮挡或抢占输入；
- 建立集中按键管理服务、输入上下文和可迁移的本地按键配置，消除 AI HUD 与镜头等系统的按键冲突；
- 修复测试假阳性，使失败必定返回非零；
- 保留重构前手动验收证据。
...
当前重构阶段的实际协作约束以《程序重构_实施约束与准入检查.md》为准：根命名空间使用 `AI_RTS`，公共接口由项目负责人评审；重构完成前当前分支只保留本地提交，不推送、不合并 main。重构完成后再采用需求/缺陷独立分支的一事一合并流程。
...
冻结期只维护保证开发和测试可用的最小 UI，不进行视觉重做，等待团队 UI 方案。

### 12.1 AI 副官界面

- 普通自定义对局默认隐藏或不实例化；
- 战役/Legacy 演示提供清晰的显示/隐藏按钮；
- 提供本地持久化设置，使开发者可以默认隐藏；
- 隐藏时释放键盘焦点、鼠标拦截和专属快捷键上下文；
- 显示状态只属于本地表现层，不进入权威比赛状态或回放；
- 不为冻结期界面增加新的布局、美术、动画、人格或对话功能。

### 12.2 按键管理

建立 C# `IInputBindingService`（名称可评审）作为 Godot `InputMap` 的集中适配器，并使用稳定的 Action ID，而不是让 HUD 直接判断物理键。
...
按键配置建议保存到 `user://controls.cfg`，包含配置版本、默认值、用户覆盖、冲突检测与恢复默认。上下文负责决定同一物理键在当前状态是否有效，例如输入框聚焦时停止单位快捷键，建筑放置时抑制不相关输入，LegacyAgent 隐藏时整体停用。

第一阶段无需制作完整按键设置 UI，只需配置文件、管理服务、冲突日志和恢复默认能力；正式 UI 等待团队设计。

## 13. 既有缺陷与回归口径

重构前项目存在已知缺陷，重构后观察到同一现象不能自动判定为迁移引入。测试报告应区分：

- `KnownBaselineDefect`：重构前已复现；
- `MigrationRegression`：重构前正常、重构后失败；
- `IntentionalBehaviorChange`：经评审的有意调整；
- `UnverifiedLegacyBehavior`：重构前未获得可靠证据。

直升机相关操作属于当前已知问题，先记录和建立复现步骤；除非阻断接口迁移，不要求在对应迁移切片中顺手修复。重构完成后必须有完整测试，再独立安排缺陷修复。
...
- 部分初始模型边界落在地图外；
- 鼠标边缘滚屏触发范围过大；
- 建造/生产交互时，鼠标靠近 UI 或屏幕边缘仍会推动镜头。

这些问题的修复应建立独立任务和验收标准，避免与 C# 迁移混合后无法判断原因。详细登记及素材审计见《程序重构_素材来源审计与既有问题登记.md》。
