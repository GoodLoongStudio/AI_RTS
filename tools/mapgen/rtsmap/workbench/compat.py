...
#: 【2026-09-24 修"随机地图其实是旧方案"】4 → 8：G2 地貌检查在 256m 图上的单套
#: 通过率约 5~6 成（台地半径/山体可用面修复后），4 套全落的概率仍有 ~4%，
#: 一落就退保底烘焙 → G3 资源无处可放 → 任务 error → 游戏退回半年前的旧图。
#: 16 套把全落概率压到 <0.2%（单套 G2+G3 联过率约三四成），代价是最多少等
#: ~45s（实测单套 2~3s；生成超时上限 900s，加载页有进度条）。
#: 【2026-09-15 提速】曾从 8 收到 4（VERIFIED_WATER_LAYOUTS 把能过检的水系↔布局
#: 排在最前，盲轮只是兜底）——该排序仍在，这里只是把兜底轮数放回 8。
LAYOUT_WALK_LIMIT = 16
