...
##
## 用法：Godot_mono_console.exe --rendering-driver opengl3 --path <AI_RTS工程根> \
...
func _run() -> void:
	...
	# 烘焙回调换入后强制同步 map（否则 map_get_closest_point 在下一次
	# physics 同步前返回 0 向量，600 帧探针窗口可能全部落在同步前）
	NavigationServer3D.map_force_update(terrain_map)
