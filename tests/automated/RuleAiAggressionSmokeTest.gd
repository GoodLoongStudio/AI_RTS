...
func _ready():
	...
	# 固定等 3 帧在启动变慢时会拿到 Nil 并中断本协程（表现为测试挂起无 completed）——
	# 与 SimpleClairvoyantAI 自身的就绪等待同口径：轮询等待而非依赖帧数时序。
	...
	_finish(match_instance, false)


func _finish(match_instance: Node, failed: bool):
	match_instance.queue_free()
	await get_tree().process_frame
	SmokeTestExit.request(get_tree(), 1 if failed else (0 if _failures == 0 else 1))
