...
func _run_test():
	...
	# HUD 缺席时不得继续解引用：以前这里抛空引用会中断整个测试协程并卡到超时
	# （既有现象：本机 headless 下 HUD/AICommandHUD 未挂载，LegacyHudVisibilitySmokeTest 同样如此）。
	# 记"无法判定"而不是冒充通过。
	if ai_hud != null:
		_check(not ai_hud.visible, "AICommandHUD 应默认隐藏")
		_check(ai_hud.control_mode == "hero", "AICommandHUD 未进入单英雄模式")
	else:
		print("  [SKIP] AICommandHUD 未挂载（既有环境现象），可见性与单英雄模式无法判定")
	...
	# 2026-09-14 用户口径：对局内 ESC 与 F10 都能唤出暂停菜单；ESC 是兜底唤出
	# （取消放置/取消目标选择等"取消"消费优先，见 EscapeRouter 不变式）。
	input_runtime.emit_signal("ActionPressed", "global.cancel")
	await get_tree().process_frame
	_check(pause_menu.visible, "Esc 应唤出暂停菜单")
	_check(get_tree().paused, "Esc 唤出菜单后应对局暂停")
	input_runtime.emit_signal("ActionPressed", "global.cancel")
	...
	input_runtime.emit_signal("ActionPressed", "global.cancel")
	_check(not pause_menu.visible, "Esc 应从（F10 打开的）暂停菜单返回对局")
	_check(not get_tree().paused, "Esc 关闭菜单后应对局恢复")
	...
	# 既有环境问题：导航自检断言会中断 Match 初始化（AirNavigation/TerrainNavigation 的
	# cell_size 自检在本机失败），导致 CampaignController 缺席。这里显式收尾，
	# 避免继续解引用把测试卡到超时（旧版会卡满超时且不留任何结论）。
	var campaign = a_match.get_node_or_null("CampaignController")
	if campaign == null:
		_failures.append("CampaignController 未加载：战役链路断言无法判定")
		print("  [SKIP] 战役链路跳过（CampaignController 未加载，属既有环境问题）")
...
## 统一收尾：提前退出路径与正常路径共用（避免遗留暂停状态或丢结论）。
func _finish_test():
