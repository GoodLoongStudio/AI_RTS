...
## 机枪塔冒烟测试（2026-09-13，反步兵快速机炮塔）：
## 逻辑必须与现有防御建筑（对地炮）一致——限速转向对准敌人才开火、命中掉血、炮管指向目标。
## 敌步兵放在炮塔 +X 方向（初始炮管朝 +Z，需要转向 ~90°）。

const MatchScene = preload("res://tests/manual/TestOneUnit.tscn")
const TurretScene = preload("res://source/match/units/MachineGunTurret.tscn")
...
func _ready():
	...
	_check(
		enemy.hp < hp_before,
		"机枪塔应在 %.0fs 内对准并击中敌步兵（hp %s → %s）" % [FIRE_TIMEOUT_SECONDS, hp_before, enemy.hp]
	)

	# 开火后炮管（+Z 约定）应指向敌人方向
	var barrel_yaw := _barrel_yaw_degrees(turret)
	var to_enemy_yaw := atan2(5.0, 0.0)
	...
	print("Machine gun turret smoke test completed: %d failure(s)" % _failures)
...
func _check(condition: bool, message: String):
	...
	push_error("Machine gun turret assertion failed: %s" % message)
