...
func _ready():
	...
	await get_tree().create_timer(0.5).timeout
	_check(enemy_tank.hp < enemy_hp_before, "恢复自由开火后 Tank 应攻击射程内敌人")

	gateway.SetFirePolicy([tank], "HoldFire", human)
	var hp_when_hold_fire_set: float = enemy_tank.hp
	var position_when_hold_fire_set: Vector3 = tank.global_position
	await get_tree().create_timer(0.3).timeout
	_check(enemy_tank.hp == hp_when_hold_fire_set, "战斗中切换停火后不应继续产生伤害")
	_check(
