...
la = pb["LeftArm"]
bm = la.bone.matrix_local
print("LOCAL_X", round(bm.col[0].x,3), round(bm.col[0].y,3), round(bm.col[0].z,3))
print("LOCAL_Y", round(bm.col[1].x,3), round(bm.col[1].y,3), round(bm.col[1].z,3))
print("LOCAL_Z", round(bm.col[2].x,3), round(bm.col[2].y,3), round(bm.col[2].z,3))

for axis, idx in [("X", 0), ("Y", 1), ("Z", 2)]:
    e = [0.0, 0.0, 0.0]
    e[idx] = radians(60)
    la.rotation_euler = Euler(e, "XYZ")
    la.keyframe_insert("rotation_euler", frame=1)
    sc.frame_set(1)
    sc.render.filepath = str(OUTD / ("axis_%s.png" % axis))
    bpy.ops.render.render(write_still=True)
    print("RENDERED", axis)
print("AXIS_DIAG_DONE")
