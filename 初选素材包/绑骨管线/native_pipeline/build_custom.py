...
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/(args.out+'.blend')))
...
if args.review:
    ...
    for name,frames in poses.items():
        ...
        for i in sorted(set([0,len(frames)//4,len(frames)//2,3*len(frames)//4,len(frames)-1])):
            ...
            sc.render.filepath=str(OUT/f'{args.out}_{name}_{i+1:03}.png');bpy.ops.render.render(write_still=True)
