...
## 狙击兵：步兵骨架 + 狙击枪（Hand_R 骨骼挂点换枪，隐藏 GLB 内嵌步枪）。
const WEAPON_SCENE := "res://assets/models/polygon-scifi/SM_Wep_Sniper_01.fbx"
...
## 把 GLB 内嵌的步枪藏掉，换成 狙击枪（挂在 Hand_R 骨骼上随动画刚性随动）。
