...
PRECONDITION_TEXT: Dict[str, str] = {
    ...
    "has_combat": "已有作战单位",
    "combat_ready": "作战单位已达小队规模",
    "below_army": "作战单位未达小队规模",
    "units_scattered": "作战单位队形散开",
    "has_resource": "有可见资源点",
    "no_enemy_intel": "尚无任何敌情",
    "has_enemy_intel": "已有敌情",
    "has_attack_cue": "有可见敌人或已知敌情",
...
PRECONDITIONS: Dict[str, Callable[[Dict[str, Any], Dict[str, Any]], bool]] = {
    ...
    "below_army": lambda facts, camp: int(facts.get("combat_count", 0)) < int(
        camp.get("army_threshold", 2) or 2),
    "units_scattered": lambda facts, camp: bool(facts.get("units_scattered")),
    ...
    "has_attack_cue": lambda facts, camp: (
        bool(facts.get("visible_enemies"))
        or int(facts.get("enemy_intel_count", 0)) > 0),
...
DECISION_NODES: Tuple[Dict[str, Any], ...] = (
    ...
    {
        "id": "D11", "node": "ATK-01", "name": "前线快集结",
        "track": campaign_mod.TRACK_MILITARY,
        "phases": (campaign_mod.PHASE_FOOTHOLD, campaign_mod.PHASE_EXPAND,
                   campaign_mod.PHASE_PRESSURE), "priority": 22,
        "preconditions": ("has_combat", "units_scattered", "not_under_attack"),
        "milestone": "",
        "build_order": (),
        "candidates": ({"skill": "GRP", "target": "location"},
                       {"skill": "AMOV", "target": "location"}),
        "success": "散开的作战单位在前线集结点附近聚拢（不是主基地门口）",
        "failure": "集结点被敌占据 → 改到更安全的前线点",
        "exit": "队形收拢后立即前压或进攻，不在主基地等待满编",
    },
    {
        "id": "D15", "node": "ADV-01", "name": "前压",
        "track": campaign_mod.TRACK_MILITARY,
        "phases": (campaign_mod.PHASE_FOOTHOLD, campaign_mod.PHASE_EXPAND,
                   campaign_mod.PHASE_PRESSURE), "priority": 48,
        "preconditions": ("has_combat", "no_visible_enemy", "not_under_attack"),
        "milestone": "",
        "build_order": (),
        "candidates": ({"skill": "AMOV", "target": "location"},),
        "success": "作战单位离开主基地圈，朝敌情或地图内侧推进",
        "failure": "路径受阻 → 换方向；遇劣势接触 → 转 D13",
        "exit": "发现可打敌人 → 转 D12；不要为散开而转回集结",
    },
    {
        "id": "D12", "node": "ATK-02", "name": "进攻", "track": campaign_mod.TRACK_MILITARY,
        "phases": (campaign_mod.PHASE_FOOTHOLD, campaign_mod.PHASE_EXPAND,
                   campaign_mod.PHASE_PRESSURE, campaign_mod.PHASE_CONVERGE), "priority": 55,
        "preconditions": ("has_combat", "has_attack_cue", "not_under_attack"),
    ...
    },
    {
        ...
        "build_order": (),
        "candidates": ({"skill": "RET", "target": "location"},
                       {"skill": "AMOV", "target": "location"}),
        "success": "与最近敌人的距离增大，或脱离敌人视野",
        "failure": "被追击且无法脱离 → 允许边打边走向脱离点",
        "exit": "脱离接触 / 到达前线集结点（不到主基地门口）",
    },
    {
        ...
        "candidates": ({"skill": "GRP", "target": "location"},
                       {"skill": "PROD", "target": "product"}),
        "success": "兵力回升且重新集结到前线集结点",
        "failure": "产能不足 → 转 D2（先恢复经济）",
        "exit": "小队可行动 → 回 D12 或 D15",
    },
)
